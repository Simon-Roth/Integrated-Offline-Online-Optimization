# bgap/data/instance_generators.py
# BGAP block-structured instance generator (m = n * d).
...
def _require_block_structure(n: int, m: int) -> int:
    """
    Enforce the bgap block structure: m = n * d.
...
def _build_block_cap_matrix(size_vec: np.ndarray, n: int, d: int) -> np.ndarray:
    """
    Build A_t^{cap} for the bgap block structure (m = n * d).
...
class BGAPInstanceGenerator(BaseInstanceGenerator):
    """
    BGAP generator that enforces the block structure (m = n * d).
    """

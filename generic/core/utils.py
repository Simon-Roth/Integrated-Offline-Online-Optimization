...
def option_is_feasible(
    feas_matrix: np.ndarray,
    feas_rhs: np.ndarray,
    option_id: int,
) -> bool:
    """
    Return True if the one-hot option satisfies A_t^{feas} x_t = b_t.
    """
    ...
    if option_id < 0 or option_id >= A.shape[1]:
        return False
...
def feasible_option_indices(
    feas_matrix: np.ndarray,
    feas_rhs: np.ndarray,
    *,
    option_ids: Optional[Iterable[int]] = None,
) -> List[int]:
    """
    Return all option indices whose one-hot selection satisfies A_t^{feas} x_t = b_t.
    """
    A = np.asarray(feas_matrix, dtype=float)
    candidates = list(range(A.shape[1])) if option_ids is None else list(option_ids)
    return [i for i in candidates if option_is_feasible(A, feas_rhs, i)]
